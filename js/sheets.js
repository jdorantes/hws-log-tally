...
// Creates tallies inside HWS Tallies / {year} / {tally name}
// Year folders are created automatically on first use each year
...
const Sheets = (() => {
  ...
  function init() {
    if (!isConfigured()) { console.warn('Google Sheets: no Client ID — offline only'); updateSyncUI(); return; }
  ...
  function signOut() { if (accessToken) google.accounts.oauth2.revoke(accessToken); accessToken = null; isSignedIn = false; updateSyncUI(); }
  function isConfigured() { return CONFIG.GOOGLE_CLIENT_ID && CONFIG.GOOGLE_CLIENT_ID !== 'YOUR_CLIENT_ID_HERE'; }
  ...
  async function getYearFolderId() {
    ...
    if (!id) { console.log('Creating year folder: ' + year); id = await createFolder(year, rootId); }
    return id;
  }
  ...
  async function createSpreadsheet(tallyName, scale) {
    ...
    const body = {
      properties: { title },
      sheets: [{ properties: { title: 'Tally', sheetId: 0 }, data: [{ startRow: 0, startColumn: 0, rowData: [{ values: headers.map(h => ({ userEnteredValue: { stringValue: h }, userEnteredFormat: { textFormat: { bold: true, foregroundColor: { red: 1, green: 1, blue: 1 } }, backgroundColor: { red: 0.05, green: 0.12, blue: 0.07 } } })) }] }] }]
    };
  ...
  function buildHeaders(scale) {
    ...
    if (scale === 'scribner') return [...base, 'Board Feet (Scribner)'];
    return [...base, 'Board Feet'];
  }
  ...
  async function appendRows(spreadsheetId, rows) {
    ...
    const values = rows.map(log => {
      ...
      if (log.scale === 'both') return [...base, log.bf ? log.bf.doyle ?? '' : '', log.bf ? log.bf.scribner ?? '' : ''];
      if (log.scale === 'doyle') return [...base, log.bf ? log.bf.doyle ?? '' : ''];
      if (log.scale === 'scribner') return [...base, log.bf ? log.bf.scribner ?? '' : ''];
      return [...base, ''];
    });
    const res = await gapi('https://sheets.googleapis.com/v4/spreadsheets/' + spreadsheetId + '/values/Tally!A:Z:append?valueInputOption=USER_ENTERED', 'POST', { values });
    return !!res;
  }
  ...
  function enqueue(spreadsheetId, rows) { const q = loadQueue(); q.push({ spreadsheetId, rows, ts: Date.now() }); saveQueue(q); updateSyncUI(); }
  ...
  function updateSyncUI() {
    const btn = document.getElementById('syncBtn'), badge = document.getElementById('syncBadge');
    if (!btn) return;
    const qc = queueCount();
    if (!isConfigured()) { btn.textContent = '☁ Sheets not configured'; btn.style.opacity = '0.4'; btn.onclick = null; return; }
    btn.style.opacity = '1';
    if (isSignedIn) { btn.textContent = qc > 0 ? '☁ Sync (' + qc + ' pending)' : '☁ Signed in to Sheets'; btn.onclick = flushQueue; }
    else { btn.textContent = qc > 0 ? '☁ Sign in to sync (' + qc + ')' : '☁ Sign in to Sheets'; btn.onclick = signIn; }
    if (badge) { badge.textContent = qc > 0 ? qc : ''; badge.style.display = qc > 0 ? 'flex' : 'none'; }
  }

  async function gapi(url, method, body) {
    try {
      const res = await fetch(url, { method, headers: { 'Authorization': 'Bearer ' + accessToken, 'Content-Type': 'application/json' }, body: body ? JSON.stringify(body) : undefined });
      if (!res.ok) { const err = await res.json().catch(() => ({})); console.error('Sheets API error:', res.status, err); return null; }
