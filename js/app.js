...
// Expose state and functions for keypad module
window.appState = () => state;
window.calcBF = calcBF;
window.toast = toast;

// Tag state for keypad — supports alphanumeric tags
let tagPrefix = '';
let tagNumLength = 5; // default padding e.g. 00001

window.setNextTag = function(fullTag, prefix, startNum, numLen) {
  tagPrefix = prefix || '';
  tagNumLength = numLen || 5;
  state.nextTag = fullTag;
  state.currentSeries++;
  saveState();
  updateHeader();
  toast('New series: ' + fullTag);
};

// Override updateHeader to handle alphanumeric tags
const _origUpdateHeader = updateHeader;
function updateHeader() {
  _origUpdateHeader();
  // Also refresh keypad if active
...
window.saveLogFromKeypad = function(lengthVal, diamVal) {
  ...
  if (!bf) { toast('Check measurements', true); return false; }
  ...
  // Increment tag — handle alphanumeric
  if (tagPrefix) {
    const numPart = parseInt(String(state.nextTag).replace(tagPrefix, '')) + 1;
    state.nextTag = tagPrefix + String(numPart).padStart(tagNumLength, '0');
  } else {
    state.nextTag = typeof state.nextTag === 'number'
      ? state.nextTag + 1
      : parseInt(state.nextTag) + 1;
  }
  ...
  if (state.spreadsheetId || (Sheets.isSignedIn() && state.tallyName)) {
    (async () => {
      if (!state.spreadsheetId && Sheets.isSignedIn()) {
        const id = await Sheets.createSpreadsheet(state.tallyName, state.scale);
        if (id) { state.spreadsheetId = id; saveState(); }
      }
      await Sheets.syncLog(state.spreadsheetId, [log]);
    })();
  }
...
// Override skipTag to refresh keypad
const _origSkipTag = skipTag;
window.skipTag = function() {
  _origSkipTag();
  if (currentMode === 'keypad') Keypad.render();
};

// ─── MODE SWITCHING ───────────────────────────────────────────────────────────
let currentMode = 'voice'; // 'voice' | 'keypad'
